...
async function getPayloadHash(fields) {
...
async function getPayloadFields(payload){
    const decodedFields = await parsePayload(payload);
    const headers = await resolver.getPayloadHeader(type, version);  
...
async function debugDownloadVerify(keyID, payload, signatureBase32NoPad) {
  let key = await resolver.getKeyId(keyID);
  if (key !== null) {
      let publicKeyPEM = key.key;
      try{
...
async function debugVerify(uri) {
    ...
    if (!await payloadTypeExist(type, version)) {
    ...
    if (formattedMessages.includes("QR was parsed sucessfully!")) {
        formattedMessages += await debugDownloadVerify(pubKeyLink, payload, signatureBase32NoPad);
    } else {
