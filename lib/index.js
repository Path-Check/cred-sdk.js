...
async function verify(pubkey, payload, signatureBase32NoPad) {
    ...
    const signatureDER = Base32URL.decode(signatureBase32NoPad);

    // Verifies Signature. 
    return key.verify(payloadSHA256, signatureDER);
}

async function sign(type, version, priKeyPEM, keyID, payloadValueArray) {
    // Load Primary Key
    ...
    const signature = Base32URL.encode(signatureDER);
...
async function signAndPack(type, version, priKeyPEM, keyID, payloadValueArray) {
...
async function hashPayload(fields) {
  ...
  const hashPassKeyBase32 = Base32URL.encode(digest);
...
async function mapHeaders(decodedFields, type, version){
    let headers = await header.parse(await resolver.getPayloadHeader(type, version));  
