...
export async function hashEndPoint(url) {
  ...
  return encode(sha256().update(image).digest());
}

export async function verifyEndPoint(url, hash) {
  return (await hashEndPoint(url)) === hash;
}
